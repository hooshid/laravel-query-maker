...
trait FiltersQuery
{
    ...
    {
        if ($this->request->get(config('query-maker.parameters.filter'))) {
            $this->getFilters();
    ...
    {
        $filters = $this->request->query(config('query-maker.parameters.filter'));
        foreach ($filters as $key => $value) {
            // find real name if column set with alias name in query
            $key = $this->findRealNameOfAliasColumn($key);

            // explode filter parameters
            $queryParameters = array_filter(explode(',', $value));

            /**
             * if parameters have value, we clean filter operator from search string
             * is_null and is_not_null does not have value
             */
            if (isset($queryParameters[1])) {
